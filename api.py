...
Yapılandırma (ortam değişkenleri — .env üzerinden):
  ...
  LLM_PROVIDER = "vllm" (varsayılan) | "gemini"
  GOOGLE_API_KEY (gemini için) | VLLM_MODEL, VLLM_BASE_URL, VLLM_API_KEY (vllm için)
...
def _build_llm():
    # (v2.1) Ollama artık KULLANILMIYOR — sistem vLLM'e taşındı (bkz.
    # bilimp_vllm.py). Varsayılan sağlayıcı "vllm"; bulut alternatifi olarak
    # "gemini" korunur.
    ...
    # vLLM, OpenAI-uyumlu bir /v1 endpoint'i sunar (bkz. bilimp_vllm.py).
    # vLLM sunucusu auth istemiyor — api_key yalnızca istemci kütüphanesinin
    # zorunlu tuttuğu bir placeholder'dır, gerçek bir sır DEĞİLDİR.
    #
    # ÖNEMLİ: Bu kod bir Docker container'ı içinde çalışabilir. "localhost"
    # burada container'ın kendisini işaret eder, host makineyi DEĞİL — bu
    # yüzden host'ta çalışan vLLM'e varsayılan base_url ile ulaşılamaz.
    # Docker Desktop'ın host'a özel adresi (host.docker.internal) varsayılan
    # olarak kullanılır. WSL/bare-metal gibi Docker'sız ortamlarda
    # VLLM_BASE_URL ile gerçek host IP'sine override edilmelidir (bkz.
    # bilimp_vllm.py:_detect_host_ip).
    return ChatOpenAI(
        model=os.getenv("VLLM_MODEL", "google/gemma-4-12B-it"),
