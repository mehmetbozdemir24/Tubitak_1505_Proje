...
Mevcut pipeline'la ilişki (v2.1 — vLLM):
  vllm_ingestion.ingest_file() DOĞRUDAN kullanılır — parçalama mantığı
  burada TEKRARLANMAZ (DRY). Bu, Docling dönüşümü + VLM görsel açıklama +
  contextual bağlam üretiminin hepsinin AYNI vLLM modelinden geçtiği "TEK
  MODEL mimarisi"dir (bkz. bilimp_vllm.py — bu pipeline oradan taşındı).
  Eski chunker.py (düz pymupdf4llm/MarkItDown tabanlı, LLM'siz) artık
  YOLDA DEĞİLDİR; tüm formatlar (pdf/docx/pptx/xlsx) Docling'in tek
  dönüşüm yolundan geçer.

  Bu modülün sorumluluğu yalnızca: dosyayı geçici olarak diske yazmak
  (Docling path bekliyor), vLLM'in ayakta olduğunu ÖNCEDEN doğrulamak
  (ensure_ctx_model — sessizce boş bağlamla devam etmek yerine hızlı
  başarısız olmak için), audience politikasını/sürüm bilgisini
  metadata'ya eklemek, embed edip Qdrant'a yazmak, ve eski sürümü
  temizlemektir.
...
from vllm_ingestion import ingest_file, ensure_ctx_model
...
def _chunk_file(file_bytes: bytes, dokuman_id: str, ext: str, audience_dict: dict) -> list:
    """
    vLLM'in ayakta olduğunu ÖNCEDEN doğrular (dosya işlenmeden önce hızlı
    başarısız olmak için — sessizce boş bağlamla devam ETMEZ), dosyayı
    geçici diske yazar, vllm_ingestion.ingest_file() ile Docling+VLM+
    contextual pipeline'ını çalıştırır, temizler.
    """
    try:
        ensure_ctx_model()
    except RuntimeError as e:
        raise DocumentIngestionError("chunking_failed", str(e))
