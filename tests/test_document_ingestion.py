...
vllm_ingestion.ingest_file (gerçek Docling+VLM+contextual ayrıştırma) ve
ensure_ctx_model (vLLM canlılık ön kontrolü) mock'lanır — burada test edilen
şey dosya ayrıştırma değil, yaşam döngüsü mantığıdır (versiyon, optimistic
...
# (v2.1) document_ingestion_service artık vllm_ingestion'ı import ediyor; o da
# Docling'i import eder (ağır, GPU-odaklı bir bağımlılık). ingest_file/
# ensure_ctx_model zaten aşağıda mock'landığından Docling'in GERÇEKTEN kurulu
# olmasına gerek yok — yalnızca import'un patlamaması için sahteleriyle
# değiştiriliyor.
...
sys.modules.setdefault("docling.datamodel.accelerator_options", MagicMock())
sys.modules.setdefault("docling_core", MagicMock())
sys.modules.setdefault("docling_core.types", MagicMock())
sys.modules.setdefault("docling_core.types.doc", MagicMock())
...
class TestCreateDocument:
    def test_successful_creation(self):
        ...
        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", return_value=_fake_chunks(3)), \
    ...
    def test_provisions_tenant_collection_if_missing(self):
        ...
        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", return_value=_fake_chunks(1)), \
    ...
    def test_rejects_when_chunker_produces_nothing(self):
        ...
        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", return_value=[]):
            with pytest.raises(DocumentIngestionError) as exc_info:
                create_document(
    ...
    def test_pptx_goes_through_same_unified_pipeline(self):
        """(v2.1) Docling ayrı bir pptx/pdf chunker ayrımı yapmaz — TÜM
        formatlar (pdf/docx/pptx/xlsx) vllm_ingestion.ingest_file() üzerinden
        AYNI pipeline'dan (Docling dönüşümü + VLM + contextual) geçer."""
        client = MagicMock()
        client.collection_exists.return_value = True
        client.scroll.return_value = ([], None)

        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", return_value=_fake_chunks(1)) as mock_ingest, \
             patch("document_ingestion_service.QdrantVectorStore"):
            create_document(
        ...
        assert mock_ingest.call_args.args[1] == "sunum.pptx"
...
class TestUpdateDocumentContent:
    ...
    def test_successful_update_increments_version(self):
        ...
        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", return_value=_fake_chunks(2)), \
             patch("document_ingestion_service.QdrantVectorStore"):
    ...
    def test_preserves_existing_audience_policy(self):
        ...
        def fake_ingest_file(path, source, size, overlap, audience):
            captured_audience.update(audience)
            return _fake_chunks(1)

        with patch("document_ingestion_service.ensure_ctx_model"), \
             patch("document_ingestion_service.ingest_file", side_effect=fake_ingest_file), \
             patch("document_ingestion_service.QdrantVectorStore"):
            update_document_content(
                client=client, collection="c", dokuman_id="rapor.pdf",
                file_bytes=b"yeni icerik", file_ext=".pdf", beklenen_versiyon=1,
